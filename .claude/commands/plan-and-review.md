...
1. **Plan phase.** Enter plan mode. If `.claude/codebase-index.md` exists, read it first for project orientation. Then explore specific areas as needed. Write a detailed implementation plan to `/plans/current-plan.md`. The plan must include:
   - Goal (1 sentence)
...
2. **Review phase.** Spawn the `staff-engineer` subagent to review `/plans/current-plan.md`.

3. **Evaluate.** Read `/plans/review.md`.
