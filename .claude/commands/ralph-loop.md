...
An external orchestrator (PowerShell script, stop hook, or manual re-invocation) calls this repeatedly.
...
4. **Identify work.** Compare the acceptance criteria in the spec against completed criteria in progress.
   Pick the next uncompleted criterion. If the human provided an argument, prioritize that criterion.
...
- `STUCK` — circuit breaker triggered (no progress or repeated errors)

## PowerShell Orchestrator Example

Save as `ralph-runner.ps1` in your project root:

```powershell
# ralph-runner.ps1 — Run /ralph-loop until done or stuck
...
Run with: `.\ralph-runner.ps1 -MaxIterations 10`
