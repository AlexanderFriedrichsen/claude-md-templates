...
- Use `/compact` proactively when you sense context is getting heavy (~70% capacity). Do not wait for auto-compaction — it is lossy.
- Use `/clear` when switching to an unrelated task. Stale context from task A degrades performance on task B.
- After compaction, re-read the plan file (`.claude/plans/current-plan.md`) to restore critical context.
...
- When context quality drops (repeated mistakes, forgotten decisions), checkpoint and restart.

## Session Bridging

Before ending any session:
1. Commit all work-in-progress (even partial — use `chore: wip` if needed)
...
- **Same fix attempted 3 times** — the approach is wrong, not the implementation. Re-plan.
- **No file changes in 3 consecutive turns** — you may be stuck in analysis paralysis. Either act or ask the human.
- **Same error appears 3 times** — the error is systemic, not a typo. Step back and understand root cause.
- **Tests deleted to make suite pass** — this is metric gaming. Revert and fix the actual code.
...
- If the current work diverges from the plan, stop and reconcile:
  - Was the plan wrong? Update it.
  - Did you drift? Course-correct back to the plan.
- Do not add features, refactor unrelated code, or "improve" things not in the plan.

## File Change Tracking

Use `git diff --name-only` (against the last commit or a reference point) to concretely track what changed per iteration. Do not rely on memory of what you edited — check the diff.
...
- If the loop supervisor (`loop-supervisor` agent) says STOP, stop. Do not override.
