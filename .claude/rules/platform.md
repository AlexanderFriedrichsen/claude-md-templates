...
- Use `pnpm` for all package operations (install, run, exec).
- `npx` works for one-off commands.
- Scripts in `package.json` should be cross-platform — avoid bash-isms like `&&` in npm scripts (use `npm-run-all` or separate scripts if needed).
- `node_modules/.bin` is on the path inside pnpm scripts automatically.
